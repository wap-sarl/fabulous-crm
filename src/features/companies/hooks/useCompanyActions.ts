...
export function companyErrorMessage(e: unknown, fallback: string): string {
  ...
  // invalid_registration_number carries the scheme's own reason after the colon.
  if (known === 'invalid_registration_number') {
    const reason = message.split('invalid_registration_number: ')[1]?.split('\n')[0];

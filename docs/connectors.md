...
3. `GET /connectors/callback` on this deployment verifies the state's signature and expiry,
   ...
   client secret and the PKCE verifier, reads the account's identity from the ID token, and
   stores the account: refresh and access tokens as ciphertext (`lib/crypto.ts`), one account
   per user and provider. The browser lands on « Intégrations » with `?connected=` or `?error=`.
4. `internal.features.connectors.actions.accessToken` serves a valid access token to the
   ...
   transient.
5. Disconnecting hides the account at once, revokes the grant at the provider when it has an
...
`appConfig` ones, which now include the connector apps' secrets.
